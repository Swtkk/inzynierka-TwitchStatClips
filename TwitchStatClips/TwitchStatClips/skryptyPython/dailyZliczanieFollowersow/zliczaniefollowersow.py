...
def fetch_all_channel_ids(conn) -> List[str]:
    ...
    # cur.execute("""
    #     SELECT ChannelId
    #       FROM dbo.StreamCurrent WITH (NOLOCK)
...
class TokenBucketLimiter:
    ...
    def _refill(self):
        ...
        if self._throttle < 1.0:
            # co sekundę zbliżamy się o ~10% do 1.0
        ...
        while True:
            async with self._lock:
                ...
                if self._tokens >= 1.0:
                    self._tokens -= 1.0
                    return
                # brak tokenu, oblicz czas oczekiwania do kolejnego refilla
    ...
    async def penalize_on_429(self, retry_after_header: Optional[str]):
        ...
        if retry_after_header and retry_after_header.isdigit():
            ...
            # brak nagłówka, użyj lekkiej kary bazowej
            sleep_s = 2
        # Obniż throttling o połowę, nie mniej niż 0.2
        async with self._lock:
...
async def fetch_followers_total(
    ...
    """
    Pobiera total followersów. Po 401 wykonuje jednorazowe odświeżenie tokenu i powtarza żądanie.
    403 traktowane jako brak uprawnień. 429 powoduje karę w limiterze oraz retry.
    """
        ...
        try:
            async with await _single_call(tok) as r:
                ...
                if r.status == 401 and not refreshed_once:
                    ...
                    refreshed_once = True
                    # powtórz próbę bez zmniejszania puli retry
                    continue

                if r.status in (401, 403):
                    log.warning(f"Brak autoryzacji {r.status} dla {broadcaster_id} — pomijam (scope/relacja).")
                    return None
...
async def fetch_avatar_url(
    ...
    Używa tego samego tokenu i limitera co followers.
    """
...
async def worker(
    ...
    while True:
        ...
        # Możesz przyspieszyć równoległym pobraniem:
        ...
        if followers is not None or avatar is not None:
            # wrzucamy do kolejki dla DB-writera
...
async def run_async(channel_ids: List[str]):
    ...
    results_q: asyncio.Queue = asyncio.Queue()   # kolejka do zapisu

    async with aiohttp.ClientSession(timeout=DEFAULT_TIMEOUT, connector=connector) as session:
        q: asyncio.Queue = asyncio.Queue()
        for ch in channel_ids:
            q.put_nowait(ch)
        # sygnały stopu dla workerów
        for _ in range(CONCURRENCY):
            q.put_nowait(None)

        # start writer-a do DB
        ...
        # czekamy aż workery skończą pobierać
        await q.join()
        for w in workers:
            await w

        # sygnał zakończenia dla writer-a
...
def main():
    log.info("=== Followers Updater (async, token-refresh, token-bucket) ===")
    conn = open_conn()
    try:
        channel_ids = fetch_all_channel_ids(conn)
    finally:
        conn.close()
    log.info(f"📦 Kanałów do aktualizacji: {len(channel_ids)}")
